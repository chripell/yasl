...
uint8_t run_p1234701ct_ble(nrf_drv_twi_t twi_master,ble_nus_t m_nus)
{
	...
	uint8_t who_am_i = p1234701ct_whoami(twi_master);
    n = snprintf((char *)ble_string, length, "p1234701ctWHOAMI:%x\r\n",who_am_i);
    send_ble_data(m_nus,(uint8_t *)ble_string,n);

    n = snprintf((char *)ble_string, length, "p1234701ctred:%hu\r\n", red);
    send_ble_data(m_nus,(uint8_t *)ble_string,n);

    n = snprintf((char *)ble_string, length, "p1234701ctgreen:%hu\r\n",green);
    send_ble_data(m_nus,(uint8_t *)ble_string,n);

    n = snprintf((char *)ble_string, length, "p1234701ctblue:%hu\r\n",blue);
    send_ble_data(m_nus,(uint8_t *)ble_string,n);
