...
uint8_t run_lis3mdl_ble(nrf_drv_twi_t twi_master,ble_nus_t m_nus){
	...
	int n;
	uint8_t who_am_i = lis3mdl_whoami(twi_master);
	n = snprintf((char *)ble_string, length, "lis3mdlid:%x\r\n",who_am_i);
    send_ble_data(m_nus,(uint8_t *)ble_string,n);

	int8_t OUT_X = lis3mdl_readOUT_X_L(twi_master);
    n = snprintf((char *)ble_string, length, "lis3mdlx:%d\r\n",OUT_X);
    send_ble_data(m_nus,(uint8_t *)ble_string,n);

	int8_t OUT_Y = lis3mdl_readOUT_Y_L(twi_master);
    n = snprintf((char *)ble_string, length, "lis3mdly:%d\r\n",OUT_Y);
	send_ble_data(m_nus,(uint8_t *)ble_string,n);

	int8_t OUT_Z = lis3mdl_readOUT_Z_L(twi_master);
    n = snprintf((char *)ble_string, length, "lis3mdlz:%d\r\n",OUT_Z);
    send_ble_data(m_nus,(uint8_t *)ble_string,n);
	return who_am_i;
