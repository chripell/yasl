...
uint8_t run_bme280_ble(nrf_drv_twi_t twi_master,ble_nus_t m_nus){
	...
	uint8_t who_am_i = bme280_whoami(twi_master);
	n = snprintf((char *)ble_string, length, "bme280id:%x\r\n",who_am_i);
    send_ble_data(m_nus,(uint8_t *)ble_string,n);

	uint8_t tempF = bme280_readTempF(twi_master);
    n = snprintf((char *)ble_string, length, "bme280tempf:%d\r\n",tempF);
    send_ble_data(m_nus,(uint8_t *)ble_string,n);


	uint8_t tempc = bme280_readTempC(twi_master);
    n = snprintf((char *)ble_string, length, "bme280tempc:%d\r\n",tempc);
    send_ble_data(m_nus,(uint8_t *)ble_string,n);
	
	memset((char *)ble_string, 0, length);
	float pressure = bme280_readFloatPressure(twi_master);
    n = snprintf((char *)ble_string, length, "press:%d\n", (int)pressure);
	send_ble_data(m_nus,(uint8_t *)ble_string,n);
    
	memset((char *)ble_string, 0, length);
	float altitude = bme280_readFloatAltitudeFeet(twi_master);
    n = snprintf((char *)ble_string, length, "bme280alt:%d\r\n",(int)altitude);
    send_ble_data(m_nus,(uint8_t *)ble_string,n);

	uint8_t humidity = bme280_readFloatHumidity(twi_master);
    n = snprintf((char *)ble_string, length, "bme280hum:%d\r\n",humidity);
    send_ble_data(m_nus,(uint8_t *)ble_string,n);
